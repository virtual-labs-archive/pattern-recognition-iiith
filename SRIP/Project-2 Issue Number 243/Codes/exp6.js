window.onload = function() {
    ...
    //FINDING THE MEAN FOR X-VALUE AND Y-VALUE OF CLASS 1
        for(i1 = 0; i1 < series1.length; i1++){
            ...
            if (series1[i1].x < xmin_1) {
                xmin_1 = series1[i1].x;
            }
            if (series1[i1].x > xmax_1) {
                xmax_1 = series1[i1].x;
            }
            if (series1[i1].y < ymin_1) {
                ymin_1 = series1[i1].y;
            }
            if (!series1[i1].y > ymax_1) 
                continue;
            ymax_1 = series1[i1].y;
        }
    ...
    //FINDING THE MEAN FOR X-VALUE AND Y-VALUE OF CLASS 2
        ...
        for(i2 = 0; i2 < series2.length; i2++){
            ...
            if (series2[i2].x < xmin_2) {
                xmin_2 = series2[i2].x;
            }
            if (series2[i2].x > xmax_2) {
                xmax_2 = series2[i2].x;
            }
            if (series2[i2].y < ymin_2) {
                ymin_2 = series2[i2].y;
            }
            if (!series2[i2].y > ymax_2)
                continue;
            ymax_2 = series2[i2].y;
        }
    ...
    function prob1(x, y){
        ...
        if(distributionFunction == 0){
            ...
            if (distributionFunction == 1 && x <= xmax_1 && x >= xmin_1 && y <= ymax_1 && y >= ymin_1) {
                r = s1Size;
            }
            console.log(r);
        return r;
    ...
    function prob2(x, y) {
        ...
        if (distributionFunction == 1 && x <= xmax_2 && x >= xmin_2 && y <= ymax_2 && y >= ymin_2) {
            r = s2Size;
        }
        console.log(r);
        return r;
